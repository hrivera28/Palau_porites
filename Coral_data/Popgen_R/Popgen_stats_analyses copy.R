...
rad_pca<-dudi.pca(rad_scale, cent=FALSE, scale=FALSE, scannf = FALSE, nf = 6)
...
#Calculate PCA with RI - OR groupings 
ggplot(rad_pca_df, aes(x=Axis2, y=Axis3,colour=Region))+
  geom_point(size=0.85)+stat_ellipse(geom="polygon", aes(fill=Region),alpha=0.25)+
  scale_colour_manual(values=c("darkblue", "red1"))+
  scale_fill_manual(values=c("darkblue","red1"))+
  guides(colour="none", fill="none")+
  xlab(paste("PC2 (", round(eval(rad_pca_eig$variance.percent[2]),2), "%)", sep=""))+
  ylab(paste("PC3 (", round(eval(rad_pca_eig$variance.percent[3]),2), "%)", sep=""))+
  theme_bw()+theme(axis.title = element_text(face="bold"),
                   axis.ticks = element_blank(),
                   axis.text = element_blank())

ggplot(rad_pca_df, aes(x=Axis3, y=Axis4,colour=Region))+
  geom_point(size=0.85)+stat_ellipse(geom="polygon", aes(fill=Region),alpha=0.25)+
  scale_colour_manual(values=c("darkblue", "red1"))+
  scale_fill_manual(values=c("darkblue","red1"))+
  guides(colour="none", fill="none")+
  xlab(paste("PC3 (", round(eval(rad_pca_eig$variance.percent[3]),2), "%)", sep=""))+
  ylab(paste("PC4 (", round(eval(rad_pca_eig$variance.percent[4]),2), "%)", sep=""))+
  theme_bw()+theme(axis.title = element_text(face="bold"),
                   axis.ticks = element_blank(),
                   axis.text = element_blank())
...
# Remove outlier loci using pca adapt package and then redo Fst

genos <- read.pcadapt("input_files/Allsamps_maf05_final_snps.vcf",type=c("vcf"))
x<-pcadapt(input=genos, K=4)
plot(x,option="screeplot")
plot(x,option="manhattan")
qval <- qvalue(x$pvalues)$qvalues
outliers <- which(qval<0.1)
length(outliers)
toremove<-locNames(rad_gl)[outliers]
...
### cores only using msat K4_pop 
cores<-read.table("input_files/core_data_correct_coreP13.txt", header=TRUE)
cores<-left_join(cores, select(metadata, Sample, Region, K4_msatpop), by="Sample")
cores<-subset(cores, !is.na(K4_msatpop))

# Using only growth data from 1994-2013 -minus 1998 and 2010 
cores_sub<-subset(cores, Year %in% c(seq(1994,1997),seq(1999,2009),seq(2011,2013)))
cores_sub %>% group_by(Sample, K4_msatpop, Region) %>% summarize(ExtRate=mean(ExtRate), 
                                                                    Density=mean(Density), 
                                                                    CalcRate=mean(CalcRate)) -> cores_run
...
(Fig4A<-ggplot(cores_run, 
           aes(x=K4_msatpop, y=Density, colour=K4_msatpop))+
